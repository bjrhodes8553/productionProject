...
public class Employee {
  ...
  static final String secondWordRegex = "(\\s[A-Za-z]+)";
  static final Pattern secondWordPattern = Pattern.compile(secondWordRegex, Pattern.MULTILINE);
  static Matcher secondWordMatcher;

  static final String firstWordRegex = "([A-Za-z]+\\s)";
  static final Pattern firstWordPattern = Pattern.compile(firstWordRegex, Pattern.MULTILINE);
  ...
  /**
   * This method takes the user's entered password and does a Regex search to make sure it has at
   * least one lowercase letter, one uppercase letter and one special character. If it does not,
   * then the password it set to pw.
   *
   * @param password
   * @return
   */
  ...
  /**
   * This method takes in the user's name and creates a username by taking the first letter of the
   * users first name and the last name, and putting the username in lowercase letters. This is done
   * using a Regex that finds a string with a space at the beginning and a word after. The string is
   * then manipulated with a string builder object, that is casted back into a string.
   *
   * @param name
   */
